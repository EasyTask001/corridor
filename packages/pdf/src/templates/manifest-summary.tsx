    ...
    <Document title={`${trip.movementNumber} manifest summary`} author={carrier.name}>
      <Page size="LETTER" style={styles.page}>
        <Masthead
          ...
          meta={[trip.movementNumber, trip.tripNumber ? `Trip ${trip.tripNumber}` : "", `Status: ${trip.status}`]}
        />
        ...
        <Section title="Carrier and filing">
          <View style={styles.grid}>
            ...
            <Field label="Port of entry" value={trip.portCode && `${trip.portCode} ${trip.portName ?? ""}`} />
        ...
        <Section title="Crew">
          <Table
            columns={[
              ...
              { key: "documents", label: "Documents", width: 20, render: (c) => c.documents.map((d) => `${d.type.replace(/_/g, " ")} ${d.number}${d.expiresOn ? ` (exp ${d.expiresOn})` : ""}`).join("; ") },
            ]}
            rows={data.crew}
            emptyText="No crew."
          />
        ...
        <Section title="Conveyance and equipment">
          <Table
            columns={[
              ...
              { key: "plates", label: "Plates", width: 22, mono: true, render: (u) => u.plates.join(", ") },
              { key: "seals", label: "Seals", width: 20, mono: true, render: (u) => u.seals.join(", ") },
            ]}
            rows={[
        ...
        <Section title="Shipments">
          <Table
            columns={[
              ...
              { key: "kind", label: "Type", width: 12, render: (s) => (s.kind ?? "").replace(/_/g, " ") },
          ...
          <Section title="Commodities">
            <Table
              columns={[
                ...
                { key: "hazmat", label: "Hazmat", width: 12, mono: true, render: (c) => c.hazmat.join(", ") },
              ]}
        ...
        <Footer left={`${carrier.name} · ${trip.movementNumber} · e-manifest summary`} generatedAt={data.generatedAt} />
      </Page>
    </Document>
  );
}
