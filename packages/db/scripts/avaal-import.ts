...
await db.transaction(async (tx) => {
  ...
  const { billingAddress, ...organizationRest } = bundle.organization!;
  await tx.insert(schema.organizations).values({ id: target.id, ...organizationRest, billingLine1: billingAddress.line1 ?? null, billingLine2: billingAddress.line2 ?? null, billingCity: billingAddress.city ?? null, billingRegion: billingAddress.region ?? null, billingPostalCode: billingAddress.postalCode ?? null, billingCountry: billingAddress.country ?? null });
  ...
  for (const x of bundle.drivers) { const id = crypto.randomUUID(); driverIds.set(x.sourceKey, id); await tx.insert(schema.drivers).values({ id, organizationId: target.id, firstName: x.firstName, lastName: x.lastName, licenseNumber: x.licenseNumber, licenseJurisdiction: x.licenseJurisdiction, dateOfBirth: x.dateOfBirth, citizenship: x.citizenship, phone: x.phone, email: x.email, gender: x.gender, hazmatEndorsement: x.hazmatEndorsement, usAddressLine1: x.address.line1 ?? null, usAddressLine2: x.address.line2 ?? null, usAddressCity: x.address.city ?? null, usAddressRegion: x.address.region ?? null, usAddressPostalCode: x.address.postalCode ?? null, usAddressCountry: x.address.country ?? null, smsOptIn: x.smsOptIn, smsPhoneAce: x.smsPhoneAce, smsPhoneAci: x.smsPhoneAci, status: x.status }); }
  ...
  for (const x of bundle.partners) { if (!x.name.trim()) continue; const id = crypto.randomUUID(); for (const key of x.sourceKeys) partnerIds.set(key, id); await tx.insert(schema.partners).values({ id, organizationId: target.id, name: x.name, type: x.type, addressLine1: x.address.line1 ?? null, addressLine2: x.address.line2 ?? null, addressCity: x.address.city ?? null, addressRegion: x.address.region ?? null, addressPostalCode: x.address.postalCode ?? null, addressCountry: x.address.country ?? null, contactName: x.contactName, contactEmail: x.contactEmail, contactPhone: x.contactPhone, status: x.status }); }
