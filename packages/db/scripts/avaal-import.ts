...
await db.transaction(async (tx) => {
  ...
  for (const [id, status] of movementStatuses) if (status !== "draft") await tx.update(schema.movements).set({ status }).where(eq(schema.movements.id, id));
