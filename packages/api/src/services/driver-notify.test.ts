...
function rows(
  over: { smsOptIn?: boolean; dispatchEmails?: string[]; entry?: string | null } = {},
): Record<string, Row[]> {
  return {
    organizations: [
      {
...
describe("runDriverNotify", () => {
  it("e-mails dispatch and the driver, texts the entry numbers, notes the delivery", async () => {
    const db = createFakeDb({
      rows: rows(),
      sqlValues: { shipperName: null, consigneeName: null },
    });
    ...
    expect(generateForMovement).toHaveBeenCalledWith(
      expect.anything(),
      { orgId: TEST_ORG_ID, userId: null },
      { movementId: MOVEMENT_ID, kind: "driver_sheet" },
    );
    ...
    expect(r).toMatchObject({
      ...
      emailed: 2,
      emailFailed: 0,
      sms: { ok: true, mode: "mock" },
    });
    const note = db.table("movementEvents").find((e) => e.eventType === "note");
    expect(note?.actorType).toBe("system");
    expect((note?.payload as { body: string }).body).toMatch(
      /e-mailed to .*SMS to \+1 905 555 0199/,
    );
    expect(
      db
  ...
  it("sends no SMS without opt-in or before an entry exists, and still reports a failed e-mail", async () => {
    ...
    const email = vi.fn(() =>
      Promise.resolve({ mode: "resend" as const, id: null, error: "bounced" }),
    );
    const sms = vi.fn();
    const r = await runDriverNotify(
      db.tx,
      TEST_ORG_ID,
      { movementId: MOVEMENT_ID, trigger: "accepted" },
      { email, sms },
    );
    expect(sms).not.toHaveBeenCalled();
    expect(r).toMatchObject({ emailed: 0, emailFailed: 2, sms: null });
    expect((db.table("movementEvents")[0]?.payload as { body: string }).body).toContain(
      "e-mail failed",
    );
  });
