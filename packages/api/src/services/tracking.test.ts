...
describe("tracking lookup input", () => {
  it("normalises and validates the carrier code and control number", () => {
    expect(trackingLookupInput.parse({ carrierCode: " pftr ", controlNumber: "paps 90001" })).toEqual({
      carrierCode: "PFTR",
      controlNumber: "PAPS90001",
    });
    expect(trackingLookupInput.safeParse({ carrierCode: "P", controlNumber: "PAPS90001" }).success).toBe(false);
    expect(trackingLookupInput.safeParse({ carrierCode: "PFTR", controlNumber: "AB" }).success).toBe(false);
    expect(trackingLookupInput.safeParse({ carrierCode: "PFTR", controlNumber: "drop;table" }).success).toBe(false);
  });
});
...
describe("tracking rate limit", () => {
  it("hashes the address and allows ten per minute, then refuses", async () => {
    ...
    expect((await trackingRateLimit(trackingKeyFor({ get: () => "198.51.100.4" }))).success).toBe(true);
  });
});
