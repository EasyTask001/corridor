...
function diff(before: unknown, after: unknown): Array<{ key: string; from: unknown; to: unknown }> {
  ...
  for (const key of keys) {
    if (JSON.stringify(b[key]) !== JSON.stringify(a[key]))
      out.push({ key, from: b[key], to: a[key] });
  }
...
export function RecordHistoryDialog({
  ...
  const history = useQuery(
    trpc.audit.forEntity.queryOptions({ entityType, entityId, limit: 50 }, { enabled: open }),
  );
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent aria-describedby={undefined} className="max-w-2xl">
        ...
        <div className="max-h-[60vh] overflow-auto text-sm">
          {history.isLoading && <p className="text-fg-secondary">Loading…</p>}
          {history.error && (
            <p role="alert" className="text-status-danger">
              {history.error.message}
            </p>
          )}
          {history.data?.length === 0 && <p className="text-fg-secondary">Nothing recorded yet.</p>}
          <ol className="space-y-3" aria-label="Record history">
            {history.data?.map((row) => {
              const changes = diff(row.before, row.after);
              return (
                <li key={row.id} className="border-b border-border-default pb-3 last:border-0">
                  <div className="flex flex-wrap items-baseline gap-x-3 text-xs text-fg-secondary">
                    ...
                    <span className="font-mono text-fg-primary">{row.action}</span>
                  </div>
                  {changes.length > 0 && (
                    <dl className="mt-1 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-xs">
                      {changes.map((c) => (
                        <div key={c.key} className="contents">
                          <dt className="font-mono text-fg-secondary">{c.key}</dt>
                          <dd className="truncate">
                            <span className="text-fg-secondary line-through">{show(c.from)}</span>{" "}
                            {show(c.to)}
                          </dd>
                        </div>
                      ))}
