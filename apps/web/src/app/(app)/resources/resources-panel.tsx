...
export function ResourcesPanel() {
  ...
  const wait = useQuery(trpc.reference.borderWait.queryOptions({ portId: portId ?? "" }, { enabled: !!portId, refetchInterval: 5 * 60_000 }));
  const tariff = useQuery(trpc.reference.tariffSearch.queryOptions({ q }, { enabled: q.trim().length >= 2 }));
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <section className="panel space-y-3 p-5" aria-label="Border wait">
        ...
        {wait.data && (
          <dl className="grid grid-cols-2 gap-2 text-sm" aria-live="polite">
            ...
            <dd className="font-mono">{wait.data.port.code} {wait.data.port.name}</dd>
            <dt className="text-ink-500">Commercial lanes</dt>
            <dd data-testid="border-wait-minutes">{wait.data.wait.lanes.commercial} min</dd>
            <dt className="text-ink-500">FAST lanes</dt>
            <dd>{wait.data.wait.lanes.fast} min</dd>
            <dt className="text-ink-500">Updated</dt>
            <dd className="text-xs text-ink-500">{new Date(wait.data.wait.updatedAt).toLocaleTimeString("en-CA")}</dd>
          </dl>
        )}
        {portId && wait.isLoading && <p className="text-sm text-ink-500">Checking…</p>}
        {!portId && <p className="text-sm text-ink-500">Pick a port to see the current wait. Refreshes every five minutes.</p>}
      </section>
      <section className="panel space-y-3 p-5" aria-label="Tariff search">
        ...
        <div>
          ...
          <Input id="tariffQ" value={q} onChange={(e) => setQ(e.target.value)} placeholder="steel coil, 7208…" />
        </div>
        <ul className="divide-y divide-ink-100 text-sm" aria-label="Tariff results">
          {tariff.data?.map((t) => (
          ...
          {tariff.data?.length === 0 && <li className="py-1.5 text-ink-500">No matches.</li>}
        </ul>
      </section>
    </div>
  );
